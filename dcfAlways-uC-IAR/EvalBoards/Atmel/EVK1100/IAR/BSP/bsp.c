...
CPU_INT32U  uiAddress;
CPU_INT32U ucData;
void mtEVBA( CPU_INT32U val)
{
	asm("mtsr 4, r12");
}
...
//       asm("NOP");
//       asm("NOP");
//       asm("NOP");		// asm nops to set calculated labels and real
//       asm("NOP");		// memory pos to the same locations  :-)
//       asm("NOP");		// it's sad but true
//       asm("NOP");
//       evbaV = (CPU_INT32U)&_evba;
//       evbaV = (CPU_INT32U)&_evba;

//       evbaV =  __builtin_mfsr(4);
//       __builtin_mtsr(4, ((int)&_evba));
//       evbaV =  __builtin_mfsr(4);
       mtEVBA(((int)&_evba));
...
/*
// pn 9.7.12, for USB we also might need to unregister an interrupt what so far was not found in micrium yet,
// so I implemented the following  (and then I saw that unregisterISR is not done ??
...
CPU_INT32U   BSP_INTC_IntUnReg(CPU_INT32U irq)
{
    ...
    BSP_INTC_Handlers[int_grp].handlers[int_id] = &BSP_INTC_IntUnhandled;
...
CPU_BOOLEAN  BSP_GPIO_PinGet (CPU_INT16U pin)
{
    ...
    gpio_port        = &AVR32_GPIO.port[pin / 32];
                                                    ...
                                                    // PN 8.june 2012, maybe better to setup port for gpio input first
                                                    // as done with the other functions below
    gpio_port->gpers = 1 << (pin % 32);  // enable GPIO
    gpio_port->odmerc = 1 << (pin % 32);  // disable open drain mode
    gpio_port->oderc = 1 << (pin % 32);   // disable output driver
    gpio_port->puerc = 1 << (pin % 32);   // switch off pullup resistor
                                          //  pull-up might be needed for some input circuits
                                          //  evtl. choose value via function-parameter             // end pn 8. june 2012
...
#warning: see remarks in bsp.h  at   #define   AdjustCycleToRTC
...
static  void  BSP_TmrReload (void)
{
...
#ifdef AdjustCycleToRTC    
    CPU_INT32U  currentRTC;    // pn 29. aug 12 added for pid correction
    OS_SEM_DATA sem_data;
    CPU_INT08U err;
#endif
      
     CPU_INT32U cyclesPerTick;                                                                   /* ... Count to avoid missing a tick interrupt due to ...   */
                                                                        /* ... accumulated error.                                   */
#if 0
    ...
    cyclesPerTick = (CPU_CLK_FREQ() / OS_TICKS_PER_SEC);
    ...
    if (intervalCnt == intervalVal) {
        ...
        if (lastRTC < currentRTC)  {    // avoid overflow situation  (very seldom)
 
            err = OSSemQuery(rtcPidSema, &sem_data);
            if (err != OS_NO_ERR) {
                err_printf("error query sem BSP_TmrReload\n");  
            }
        
            if (sem_data.OSCnt == waitingForNewRTCValue ) {         
                intervalDurationRTC = currentRTC - lastRTC; 
                OSSemSet(rtcPidSema, nextRTCValuesSet , &err);
                if (err != OS_NO_ERR) {
                    err_printf("error setting Job sema nextRTCValuesSet BSP_TmrReload\n");
                }
            } else if (sem_data.OSCnt == nextCorrectionValuesSet) {    
                   cycleCorrection = nextCycleCorrection;
                   OSSemSet(rtcPidSema, nextCorrectionValuesReceived , &err);
                    if (err != OS_NO_ERR) {
                        err_printf("error setting Job sema nextRTCValuesSet BSP_TmrReload\n");
                   }                
            }
