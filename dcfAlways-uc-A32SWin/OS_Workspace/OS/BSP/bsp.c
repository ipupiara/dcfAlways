...
// PN 31.may 2012 now defined in sdramc/EVK1100/evk1100.h
...
void neededForDebugKakke (CPU_INT32U adr)
{
	adr = adr +20;
}
...
CPU_INT32U  BSP_INTC_IntReg (CPU_FNCT_VOID handler, CPU_INT32U irq, CPU_INT32U int_level)
{
    ...
    for (i1=0; i1 < 8; ++ i1){
    	adr = OSIntPrioReg[i1];  // pn just for debuggin (checking the asm chaos)
//    	neededForDebugKakke(adr);
//    	adr = adr + 0x10;
    }
...
CPU_BOOLEAN  BSP_GPIO_PinGet (CPU_INT16U pin)
{
    ...
    gpio_port->oderc = 1 << (pin % 32);     // PN, 25.5.2012
    gpio_port->odmerc = 1 << (pin % 32);		// I prefer to set all the needed values here

    gpio_port->ovrc  =	1 << (pin % 32);    // and let's also set ovrc, not urg. needed, but ok
